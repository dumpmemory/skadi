...
func (s *Service) AgentDelete(aid string) error {
	...
	// prepare secret for cache clean
	secret, err := s.AgentSecret(aid)
	if err != nil {
		s.log.Errorw("find agent secret failed when delete agent", "error", err)
	}
